...
import { timelineItemSchema } from '@/validations/admin';

export const PUT = withAdminAuth(async (req, { params }) => {
  const { id } = await params;
  const body = await req.json().catch(() => null);
  const parsed = timelineItemSchema.partial().safeParse(body);
  if (!parsed.success) {
  ...
  try {
    const item = await prisma.timelineItem.update({ where: { id }, data: parsed.data });
